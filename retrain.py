...
def add_final_training_ops(class_count, final_tensor_name, bottleneck_tensor):
  ...
  #  train_step = optimizer.minimize(cross_entropy_mean, var_list=tf.compat.v1.trainable_variables())
  with tf.name_scope('train'):
      optimizer = tf.optimizers.SGD(FLAGS.learning_rate)

      # 手动计算并应用梯度
      with tf.GradientTape() as tape:
          # 重新计算损失值，确保它在 GradientTape 中
          logits = tf.matmul(bottleneck_input, layer_weights) + layer_biases
          cross_entropy = tf.nn.softmax_cross_entropy_with_logits(
              labels=ground_truth_input, logits=logits)
          cross_entropy_mean = tf.reduce_mean(cross_entropy)
      ...
      # 计算损失值相对于可训练变量的梯度
      gradients = tape.gradient(cross_entropy_mean, [layer_weights, layer_biases])

      # 使用计算的梯度更新变量
      train_step = optimizer.apply_gradients(zip(gradients, [layer_weights, layer_biases]))
...
def main(_):
  ...
  with tf.compat.v1.Session(graph=graph) as sess:
    ...
    train_writer = tf.summary.create_file_writer(FLAGS.summaries_dir + '/train')  # 改动
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      # step. Capture training summaries for TensorBoard with the `merged` op.
        ...
        _, train_loss, train_accuracy = sess.run(
            [train_step, cross_entropy, evaluation_step],  # 改动：删除了 merged
            feed_dict={bottleneck_input: train_bottlenecks,
                       ground_truth_input: train_ground_truth})

        # 将 summary 写入训练日志
        with train_writer.as_default():  # 改动：上下文管理器写入 summary 数据
            tf.summary.scalar('cross_entropy', train_loss, step=i)  # 改动
            tf.summary.scalar('accuracy', train_accuracy, step=i)  # 改动
            train_writer.flush()  # 改动：确保数据立即写入
      ...
      if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
        ...
        with validation_writer.as_default():  # 改动：上下文管理器写入 summary 数据
            tf.summary.scalar('validation_loss', validation_loss, step=i)  # 改动
            tf.summary.scalar('validation_accuracy', validation_accuracy, step=i)  # 改动
            validation_writer.flush()  # 改动：确保数据立即写入
